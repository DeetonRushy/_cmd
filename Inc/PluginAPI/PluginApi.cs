...
namespace cmd
{
    class api
    {
        ...
        public static bool CreateCommand(string _visable, string _internal, Func<RetType> fmt, string desc)
        {
            var init = AssemblyName.GetAssemblyName("..\\..\\cmd.exe"); // edit this to fit your path/structure
            var assembly = Assembly.Load(init);

            var type = assembly.GetType("cmd.CommandExecutor");
...
namespace TestDll
{
    ...
    class Init_Class
    {
        ...
        public void Initialize()
        {
            ...
            cmd.api.CreateCommand("suitehelp", "__suite_help", () =>
            {
                // BARE bones example, but literally anything can be done here.
                // Lets just tell them we have no help to provide as we're a 
                // Beta plugin.

                cmd.api.WriteLine("g_suite", "There is no help to provide right now as we're in beta!");
                return cmd.api.RetType._C_SUCCESS;
